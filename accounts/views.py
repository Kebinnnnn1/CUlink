...
class VerifyEmailView(View):
    def get(self, request):
        ...
        messages.success(request, 'Email verified! You can now use all the features.')
        return redirect('login')
...
class DashboardView(View):
    def get(self, request):
        ...
        all_items = WorkspaceItem.objects.filter(doc__owner=request.user)
        total_docs   = docs.count()
        open_tasks   = all_items.filter(is_done=False).count()
        done_goals   = all_items.filter(doc__type='goal', is_done=True).count()
        total_goals  = all_items.filter(doc__type='goal').count()

        # Upcoming due dates (within 7 days, not done)
        upcoming = list(
            WorkspaceItem.objects.filter(
                doc__owner=request.user,
                due_date__isnull=False,
                is_done=False,
                due_date__lte=week_ahead,
            ).select_related('doc').order_by('due_date')[:8]
        )

        # Todo docs for quick-capture dropdown
        todo_docs = list(docs.filter(type='todo').values('pk', 'title')[:10])

        # Recent docs
        recent_docs = list(docs.order_by('-updated_at')[:5])
...
class ConversationView(View):
    ...
    def get(self, request, username):
        ...
        return render(request, 'accounts/chat_conversation.html', {
            ...
            'me_avatar_url':    me.avatar.url if me.avatar and me.avatar.name else '',
            'other_avatar_url': other.avatar.url if other.avatar and other.avatar.name else '',
        })
...
def create_post(request):
    ...
    # Support both single-file 'image' and multi-file 'images' uploads
...
def workspace_view(request):
    ...
    for d in docs:
        doc_dict = {
            ...
            'title': d.title,
            'color': d.color,
            'items': [_serialize_item(i) for i in d.items.all()],
        }
...
@login_required(login_url='/login/')
@verified_required
def workspace_sync(request):
    """Lightweight JSON endpoint — returns all docs+items for frontend re-sync."""
    docs = WorkspaceDoc.objects.filter(owner=request.user).prefetch_related('items')
    docs_data = []
    for d in docs:
        doc_dict = {
        ...
        docs_data.append(doc_dict)
    return JsonResponse({'ok': True, 'docs': docs_data})
